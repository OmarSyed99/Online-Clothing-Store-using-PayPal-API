...
let textDiv2 = document.getElementById("text-area2")
textDiv2.innerHTML = 'test'
...
socket.on('redStoneData', function(data) {
  console.log("data: " + data)
  //console.log("typeof: " + typeof data)
  let stoneData = JSON.parse(data)
  homeStones.elementAt(stoneData.index).setLocation(stoneData)
  drawCanvas()
})
socket.on('yellowStoneData', function(data) {
  console.log("data: " + data)
  //console.log("typeof: " + typeof data)
  let stoneData = JSON.parse(data)
  visitorStones.elementAt(stoneData.index).setLocation(stoneData)
...
socket.on('Collision', function(data) {
  ...
  //console.log("typeof: " + typeof data)
  let stoneData = data
  setOfCollisions.addCollision(stoneData)

  drawCanvas()
})
socket.on('playerTurn', function(data) {
  let pTurn = JSON.parse(data)
  whosTurnIsIt = pTurn.turn

  drawCanvas()
})
socket.on('disable', function(data) {
  let btnDisable = JSON.parse(data)
  if (btnDisable.player == "home"){
    document.getElementById("JoinAsHomeButton").disabled = btnDisable.off
    document.getElementById("JoinAsHomeButton").style.backgroundColor = btnDisable.color
  }else if( btnDisable.player == "visitor"){
    document.getElementById("JoinAsVisitorButton").disabled = btnDisable.off
    document.getElementById("JoinAsVisitorButton").style.backgroundColor = btnDisable.color
  }
...
function handleMouseDown(e) {
  ...
  if(stoneBeingShot === null){
    if(iceSurface.isInShootingCrosshairArea(canvasMouseLoc)){
      ...
      //textDiv.innerHTML = textDiv.innerHTML + " " + stoneBeingShot.getLocation().x
  ...
  let playerT = {
    turn: shootingQueue.front().getColour()
  }
  let jsonTurn = JSON.stringify(playerT)
  socket.emit('playerTurn', jsonTurn)
  console.log("turn" + whosTurnIsIt)
...
function handleTimer() {
  ...
  for (let stone1 of allStones.getCollection()) {
    for (let stone2 of allStones.getCollection()) {
      ...
      if ((stone1 !== stone2) && stone1.isTouching(stone2) && (stone1.isStoneMoving() || stone2.isStoneMoving())){
        ...
        socket.emit('Collision', new Collision(stone1,stone2))
      }
    }
  }
  ...
  if(stoneBeingShot != null){
    ...
    for(let i =0; i<STONES_PER_TEAM; i++){
      if((homeStones.elementAt(i).getLocation().x == stoneBeingShot.getLocation().x) && (homeStones.elementAt(i).getLocation().y == stoneBeingShot.getLocation().y)){

        let stoneThrown = {
          x: homeStones.elementAt(i).getLocation().x ,
          y: homeStones.elementAt(i).getLocation().y ,
          index: i
        }
        let jsonStone = JSON.stringify(stoneThrown)
        socket.emit('redStoneData', jsonStone)
      }

    }
    for(let i =0; i<STONES_PER_TEAM; i++){
      if((visitorStones.elementAt(i).getLocation().x == stoneBeingShot.getLocation().x) && (visitorStones.elementAt(i).getLocation().y == stoneBeingShot.getLocation().y)){
        let stoneThrown1 = {
          x: visitorStones.elementAt(i).getLocation().x ,
          y: visitorStones.elementAt(i).getLocation().y ,
          index: i
        }
        let jsonStone1 = JSON.stringify(stoneThrown1)
        socket.emit('yellowStoneData', jsonStone1)
      }
    }
...
function handleSubmitButton() {
  ...
  if (userText && userText !== '') {
    ...
    $('#userTextField').val('') //clear the user text field
      ...
      socket.emit('redStoneData', jsonString)
...
function handleJoinAsHomeButton(){
  ...
  let jsonButton = JSON.stringify(homeButton)
  socket.emit('disable', jsonButton)
